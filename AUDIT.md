...
Moved to [MANUAL_TESTS.md](MANUAL_TESTS.md), which is the tracking list: **74 tests, grouped by
what they need set up** (a production build, a second client, real media devices, a real plugin)
with a checkbox each. The rows kept their M numbers, which findings in this file reference, so
those numbers must not be renumbered.
