...
let discoveryCache: {
  value: Awaited<ReturnType<typeof client.discovery>>;
  issuer: string;
  expiresAt: number;
} | null = null;
...
export const getOidcConfig = async () => {
  ...
  if (!isLocal && issuerUrl.protocol !== 'https:') {
    throw new Error(
      `Security Error: OIDC Issuer must use HTTPS for non-local host: ${issuerUrl.hostname}`
    );
  }

  if (
    discoveryCache &&
    discoveryCache.issuer === config.oidc.issuer &&
    Date.now() < discoveryCache.expiresAt
  ) {
  ...
  if (config.oidc.caCertPath) {
    try {
      ...
      console.error(
        `OIDC Config Error: Failed to read CA file at ${config.oidc.caCertPath}.`
      );
    }
  }
...
export const oidcLogin = async (
  ...
  try {
    ...
    const sessionData = JSON.stringify({
      ...
      state,
      nonce,
      redirectOrigin: refererOrigin
    });

    // Set OIDC session cookie
    res.setHeader(
      'Set-Cookie',
      `__Host-oidc_session=${encodeURIComponent(sessionData)}; HttpOnly; Secure; SameSite=Lax; Max-Age=300; Path=/`
    );
...
export const oidcCallback = async (
  ...
  try {
    ...
    if (
      !redirectOrigin ||
      !config.oidc.allowedOrigins.includes(redirectOrigin)
    ) {
    ...
    const needsUserInfo =
      ...
      (!!config.oidc.displayNameClaim &&
        !idTokenClaims[config.oidc.displayNameClaim]);
    ...
    if (config.oidc.requiredGroups.length > 0) {
      const userGroups = (
        (mergedClaims[config.oidc.groupsClaim] as string[]) || []
      ).map((g) => g.toLowerCase());
      const hasRequired = config.oidc.requiredGroups.some((r) =>
        userGroups.includes(r.toLowerCase())
      );
      if (!hasRequired) {
    ...
    const identity = ((mergedClaims.email as string) ||
      idTokenClaims.sub) as string;
    const user = await syncUserWithDatabase(identity, mergedClaims);

    const appToken = jwt.sign({ userId: user.id }, await getServerToken(), {
      expiresIn: '1d'
    });
    ...
    const clearSession =
      '__Host-oidc_session=; Max-Age=0; Path=/; HttpOnly; Secure; SameSite=Lax';
...
function resolveDisplayName(claims: Record<string, unknown>): string {
  ...
  return (
    (claims[config.oidc.usernameClaim] as string) ?? (claims.sub as string)
  );
}
...
async function syncUserWithDatabase(
  ...
  let user =
    (await getUserByOidcSub(sub)) ?? (await getUserByIdentity(identity));
  ...
  if (!user) {
    ...
    if (!insertedUser) {
      throw new Error(
        'Failed to create user: Database insert returned no data.'
      );
    }
...
async function applyRoleMappings(userId: number, claims: any) {
  ...
  const oidcGroups = ((claims[config.oidc.groupsClaim] as string[]) || []).map(
    ...
    if (oidcGroups.includes(oidcRole.toLowerCase())) {
      const dbRole = allDbRoles.find(
        (r: { id: number; name: string }) =>
          r.name.toLowerCase() === localRole.toLowerCase()
      );
      if (dbRole) targetRoleIds.push(dbRole.id);
    }
  }
  const uniqueTargetRoleIds = [...new Set(targetRoleIds)];
  const userCurrentRoles = await db.query.userRoles.findMany({
    where: eq(userRoles.userId, userId)
  });

  if (config.oidc.enforceOidcRoles) {
    const mappedRoleNames = Object.values(rolesMapping).map((name) =>
      name.toLowerCase()
    );
    const mappedDbRoles = allDbRoles.filter((r: { id: number; name: string }) =>
      mappedRoleNames.includes(r.name.toLowerCase())
    );
    const mappedDbRoleIds = mappedDbRoles.map((r) => r.id);
    const userCurrentRoleIds = userCurrentRoles.map((r) => r.roleId);

    const rolesToRemove = userCurrentRoleIds.filter(
      (id) => mappedDbRoleIds.includes(id) && !uniqueTargetRoleIds.includes(id)
    );
    ...
    const rolesToRemove = oidcManagedRoleIds.filter(
      (id) => !uniqueTargetRoleIds.includes(id)
    );
    if (rolesToRemove.length > 0) {
      await db
        .delete(userRoles)
  ...
  const rolesToAdd = uniqueTargetRoleIds.filter(
