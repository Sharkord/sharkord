...
const PluginLogsDialog = memo(({ isOpen, close, pluginId, pluginName }: TPluginLogsDialogProps) => {
  ...
  return (
    <Dialog open={isOpen}>
      <DialogContent onInteractOutside={close} close={close} className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{pluginName ?? pluginId}</DialogTitle>
        </DialogHeader>

        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <span>{t('infoLabel')} {counts.info}</span>
          <span>{t('errorsLabel')} {counts.errors}</span>
          <span>{t('debugLabel')} {counts.debug}</span>
          <span className="ml-auto">{t('totalLogs', { count: logs.length })}</span>
        </div>

        <div className="flex items-center gap-2">
          <span className="text-xs text-muted-foreground">{t('showLabel')}</span>
          <Select
